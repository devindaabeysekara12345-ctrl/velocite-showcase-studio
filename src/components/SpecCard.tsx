...
export function SpecCard({ label, value, unit, delay = "0s" }: Props) {
  return (
    <div
      className="glass rounded-xl sm:rounded-2xl p-3 sm:p-5 w-full lg:w-56 relative overflow-hidden animate-float-slow"
    ...
    >
      ...
      <div className="flex items-baseline gap-1">
        <span className="font-display text-2xl sm:text-4xl font-bold text-foreground tabular-nums">
          {value}
        </span>
        <span className="text-[10px] sm:text-xs font-mono text-neon">{unit}</span>
      </div>
